...
class DetectorMovimiento:
    ...
    def _actualizar_candidatos(self, blobs: List[tuple[int, int, int, int]]) -> None:
        ...
        for x, y, w, h in blobs:
            ...
            for cid, cand in self._candidatos.items():
                if cid in usados:
                    continue
            ...
            if mejor_id is not None:
                ...
                cand = self._candidatos[mejor_id]
                cand.x1 = x
                cand.y1 = y
                cand.x2 = x + w
                cand.y2 = y + h
                ...
                self._candidatos[cid] = CandidatoMovimiento(
                    id=cid,
                    x1=x,
                    y1=y,
                    x2=x + w,
        ...
        # Incrementar frames_sin_match de los no usados
        para_borrar: List[int] = []
        for cid, cand in self._candidatos.items():
            if cid not in usados:
                cand.frames_sin_match += 1
                if cand.frames_sin_match > self.max_frames_sin_match:
                    para_borrar.append(cid)

        for cid in para_borrar:
            del self._candidatos[cid]
    ...
    def actualizar(self, frame: np.ndarray) -> List[Detection]:
        ...
        for cand in self._candidatos.values():
            ...
            if cand.frames_vivos < self.min_frames_vivos:
                continue

            # Desplazamiento total desde el inicio
            desplazamiento = hypot(cand.cx - cand.inicio_cx, cand.cy - cand.inicio_cy)
